...
app.get('/admin.html',     (q,r) => r.sendFile(path.join(__dirname,'public','admin.html')));
app.get('/dashboard.html', (q,r) => r.sendFile(path.join(__dirname,'public','dashboard.html')));
app.get('/health.html',    (q,r) => r.sendFile(path.join(__dirname,'public','health.html')));
...
function loadDB() {
  try {
    if (!fs.existsSync(DB_FILE)) {
      const blank = { users:[], keys:[], audit:[], payments:[] };
    ...
    return d;
  } catch(e) { console.error('[DB]',e.message); return {users:[],keys:[],audit:[],payments:[]}; }
}
...
function makeKey(planId, userId, name, email) {
  const p = PLANS[planId]; if (!p) throw new Error('Unknown plan: '+planId);
...
async function sendEmail(to, subject, html) {
  if (!mailer) { console.log('[Email] Skipped:', to); return false; }
  try {
...
app.post('/api/user/register', async (req, res) => {
  ...
  if (db.users.find(u=>u.email===el)) return res.status(400).json({ error:'Email already registered — please sign in' });
...
app.post('/api/payment/create-order', requireAuth, async (req, res) => {
  ...
  if (!razorpay) return res.status(503).json({ error:'Payment gateway not ready', fallback:true,
...
app.post('/api/payment/verify', requireAuth, async (req, res) => {
  ...
  const db = loadDB(), user = db.users.find(u=>u.id===req.user.id), plan = PLANS[planId];
  const keys = [];
...
app.post('/api/activate', (req, res) => {
  ...
  res.json({ valid:true, plan:key.plan, tier:key.plan, // tier kept for old agents
...
app.get('/api/admin/users', requireAdmin, (req,res) => res.json({ users:loadDB().users.map(u=>({
...
app.post('/api/admin/generate-key', requireAdmin, async (req,res) => {
  ...
  for (let i=0; i<n; i++) {
    const k=makeKey(plan,null,customerName,customerEmail); k.notes=notes||null;
    db.keys.unshift(k); keys.push(k);
  }
...
app.get('/api/health', (req,res) => {
  ...
  res.json({ status:'ok', version:'4.0.0', db:DB_FILE,
    ...
    email:!!mailer, razorpay:!!razorpay, time:new Date().toISOString() });
});
...
async function startServer() {
  ...
  console.log('  Persistent:', DATA_DIR==='/data' ? 'YES' : 'NO — add Volume at /data');
  await initEmail();
  await initRazorpay();
